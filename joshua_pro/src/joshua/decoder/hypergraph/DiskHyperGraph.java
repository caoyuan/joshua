 ...
 * @version $LastChangedDate: 2010-02-10 10:46:05 -0500 (Wed, 10 Feb 2010) $
 */
...
//===============================================================
	...
	/*
	 * for writing hyper-graph: 
	 * 		(1) saving each hyper-graph;  
	 * 		(2) remember each regular rule used; 
...
//===============================================================
	...
	//EG: #I 52 0 8 [S] 3 ST 0_OOV |||_OOV for a -S- for a throbbing pain
		...
		this.itemsWriter.write(
			new StringBuffer()
				...
				.append(
					...
					? item.getDPStates().get(this.LMFeatureID).getSignature(this.symbolTable, true)
					: NULL_ITEM_STATE )
				.append("\n")
				.toString()
			);
		
		//THEN START TO WRITE THE HYPEREDGE INFO LEADING TO THIS NODE
	...
	private void writeHyperedge(HGNode node, HyperEdge edge) throws IOException 
	{
		...
		if (null == edge.getAntNodes())  //NO ANTECEDENT NODES 
		{
			s.append(0);
		} 
		else 
		{
			final int qtyItems = edge.getAntNodes().size();  //NUMBER OF ANTECEDENT NODES
			s.append(qtyItems);
			for (int i = 0; i < qtyItems; i++) 
			{
				s.append(' ').append(this.itemToID.get(edge.getAntNodes().get(i) ));  //NODE ID
			}
		}
		
		s.append(' ').append(ruleID);  //RULE ID
		...
		{
			s.append( createModelLogPLine(node, edge) );  //ADD FEATURE VALUES
		}
	...
	public String createModelLogPLine(HGNode parentNode, HyperEdge edge)
	{
		...
		{
			transitionLogPs = ComputeNodeResult.computeModelTransitionLogPs(this.featureFunctions, edge, parentNode.i, parentNode.j, this.sentID);
		}
		else
		{
...
//===============================================================
	...
	private HGNode readNode() {
		...
		if (fds.length != 2) {
			System.out.println("DEBUG: Wrong line: " + line);
			throw new RuntimeException("wrong item line");
			
			//MODIFIED FOR TEMPORARY USE
			
			//line = "#I 1 3 5 [X] 1 ST X -S- X";
			//fds = line.split(ITEM_STATE_TAG);
		}
