...
public class Optimizer {
  ...
  public double[] run_Optimizer() {
    ...
    try {
      ...
      myClassifier.setClassifierParam(classifierParam); // set classifier
                                                        // parameters
      ...
      double initMetricScore = computeCorpusMetricScore(initialLambda); // compute
                                                                        // the
                                                                        // initial
                                                                        // corpus-level
                                                                        // metric
                                                                        // score
      double finalMetricScore = computeCorpusMetricScore(finalLambda); // compute
                                                                       // the
                                                                       // final
                                                                       // corpus-level
                                                                       // metric
        ...
        if (nbestFormat.equals("dense")) {
          ...
          if (!trainMode.equals("3")) {
            ...
            for (int f = 0; f < feat_str.length; f++) {
              ...
              new_feat_id = Integer.parseInt(feat_info[0]) - regParamDim; // for
                                                                          // mode
                                                                          // 3,
                                                                          // re-index
                                                                          // the
  ...
  public Vector<String> process_Params() {
    Vector<String> allSamples = new Vector<String>(); // to save all sampled
                                                      // pairs

    // sampling
    Vector<String> sampleVec = new Vector<String>(); // use String to make
  ...
  private Vector<String> Sampler(int sentId) {
    ...
    HashMap<String, Double> candScore = new HashMap<String, Double>(); // metric(e.g
                                                                       // BLEU)
                                                                       // score
                                                                       // of all
                                                                       // candidates

    // extract all candidates to a string array to save time in computing BLEU
    // score
    ...
    if (Tau < candCount * (candCount - 1)) // otherwise no need to sample
    {
      int j1, j2;
      for (int i = 0; i < Tau; i++) {
        // here the case in which the same pair is sampled more than once is
        // allowed
        // otherwise if Tau is almost the same as candCount^2, it might take a
        // lot of time to find
        // Tau distinct pairs
    ...
    for (String key : acceptedPairSort.keySet()) {
      ...
      if (!added.contains(key)) {
        ...
        if (evalMetric.getToBeMinimized()) // if smaller metric score is
                                           // better(like TER)
  ...
  private double Alpha(double x) {
    return x < metricDiff ? 0 : 1; // default implementation of the paper's
                                   // method
  ...
  private double metricDiff; // metric difference threshold(to select the
