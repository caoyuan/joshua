...
\subsection{Investigation of CCG combinators and reordering}

When assigning CCG labels to translation rules, we may extract rules such as
\begin{equation*}
A \to \langle A/B_1 \, B_2 ; A/B_1 \, B2 \rangle
\end{equation*}
which simulate the application of combinators (in this case, function application).

If we were to restrict our extracted grammar to non-hierarchical rules --- like the lexical entries of Table \ref{table:lexicon}, but bilingual --- and combinator-simulating rules, the resulting model would simulate a synchronous CCG.

An important question, then, would be when combinators are allowed to reorder subtrees in a translation. We hope to build off of work by Mylonakis and Sima'an \shortcite{mylonakis2011} and their development of Hierarchical Reordering SCFGs.
...
If we could build a translation model based on a CCG, it would be plausible to try to include some semantic information to guide the translation process. 
...
Our model allows us to extract almost as many rules as the SAMT model, but the label set is an order of magnitude smaller. We find that these linguistically-plausible labels give comparable translation performance to the heuristic SAMT labels with more efficient decoding.
