...
public class MIRACore {
  ...
  // docOfSentence is 0-indexed, as are the documents (i.e. first doc is indexed
  // 0)
  ...
  // only [1] and [2] are needed for optimization. The rest are only needed for
  // an output message.
  ...
  // 0: no normalization, 1: "NIST-style" tokenization, and also rejoin 'm, 're,
  // *'s, 've, 'll, 'd, and n't,
  // 2: apply 1 and also rejoin dashes between letters, 3: apply 1 and also drop
  ...
  private int numParamsInFile; // = THE NON-EMPTY LINE NUM IN THE PARAMS
  ...
  // (*) minThValue and maxThValue can be real values as well as -Infinity and
  // +Infinity
  ...
  private int maxMIRAIterations, minMIRAIterations, prevMERTIterations;
  ...
  // (but see minMIRAIterations above and stopMinIts below)

  private int stopMinIts;
  // some early stopping criterion must be satisfied in stopMinIts *consecutive*
  // iterations
  // before an early exit (but see minMIRAIterations above)
  ...
  // name of evaluation metric optimized by MERT, possibly with "doc-level "
  // prefixed

  private String[] metricOptions;
  // options for the evaluation metric (e.g. for BLEU, maxGramLength and
  // effLengthMethod)
  ...
  // should the iteration number be passed as an argument to
  ...
  private double scoreRatio = 4.0; //SCLALE SO THAT MODEL_SCORE/METRIC_SCORE = scoreRatio
  ...
  public MIRACore(String[] args) {
    EvaluationMetric.set_knownMetrics();
    processArgsArray(args);
    initialize(0);
  }

  public MIRACore(String configFileName) {
    EvaluationMetric.set_knownMetrics();
  ...
  private void initialize(int randsToSkip) {
    ...
    // COMBINED REFERENCE FILE NAME(AUTO GENERATED)
    ...
    // SentenceInfo.createV(); // uncomment ONLY IF using vocabulary
    // implementation of SentenceInfo
      ...
      */
      ...
      for (int c = 1; c <= numParamsInFile; ++c) {
        ...
        if (!isOptimizable[c]) {
          ...
          print(" Yes\t\t", 1);
          // print("[" + minThValue[c] + "," + maxThValue[c] + "] @ " +
  ...
  public void run_MIRA() {
    run_MIRA(minMIRAIterations, maxMIRAIterations, prevMERTIterations);
  }
  ...
  public void run_MIRA(int minIts, int maxIts, int prevIts) {
    ...
    // IF NO DEFAULT LAMBDA IS MIRAVIDED
    if (randInit) {
  ...
  public double[] run_single_iteration(int iteration, int minIts, int maxIts, int prevIts,
    ...
    boolean done = false;
    retA[2] = 1; // will only be made 0 if we don't break from the following
                 // loop
    ...
    while (!done) { // NOTE: this "loop" will only be carried out once
      ...
      // run the decoder on all the sentences, producing for each sentence a set
      // of
      // sizeOfNBest candidates, with numParams feature values for each
      // candidate
      ...
      String[] decRunResult = run_decoder(iteration); // iteration passed in
                                                      // case fake decoder will
                                                      // be used
      ...
      if (saveInterFiles == 1 || saveInterFiles == 3) { // make copy of
                                                        // intermediate config
                                                        // file
        ...
        if (trainingMode.equals("2") || trainingMode.equals("3"))
          if (!copyFile(discDefFile, discDefFile + ".MIRA.it" + iteration)) {
            println("Warning: attempt to make copy of sparse feature definition file (to create"
                + discDefFile + ".MIRA.it" + iteration + ") was unsuccessful!", 1);
          }
      }

      // SAVE output.nest.MIRA.it?
      if (saveInterFiles == 2 || saveInterFiles == 3) { // make copy of
      ...
      // INITIALIZE SET OF WEIGHT VECTOR CANDIDATES, THE 1ST ONE BEING THE
      // lambda FROM LAST ITERATION
      // OTHERS ARE RANDOMLY GENERATED LAMBDAS
      // set initialLambda[][]

      System.arraycopy(lambda, 1, initialLambda[1], 1, numParams);

      //THIS PART NOT USED FOR NOW
      ...
      // WRONG
      // int[][][] allCand_suffStats = new
      // int[numSentences][sizeOfNBest][suffStatsCount]; //SUFF STATS FOR ALL
      // CANDIDATES
      ...
      // allCand_suffStats[a][b][c] = -1; //IF N-BEST IS NOT SUFFICIENT, YOU
      // KNOW WHICH VALUE IS NOT UPDATED
      // }
      // }
      ...
      // YOU CAN CONSIDER ALL CANDIDATES FROM ITER 1, OR FROM ITER
      // (iteration-prevIts) TO CURRENT ITERATION
      ...
      try {
        ...
        // (index 0 is not used; no corresponding index for the current
        // iteration)
        ...
        BufferedReader inFile_statsCurrIt = null; // will only be used if
                                                  // statsCurrIt_exists below is
                                                  // set to true
        PrintWriter outFile_statsCurrIt = null; // will only be used if
                                                // statsCurrIt_exists below is
                                                // set to false
        ...
        // OUTPUT THE 5^TH 6^TH TEMP FILE, BUT WILL BE DELETED AT THE END OF THE
        // FUNCTION
        ...
        // BUT IF THE USER SPECIFIED TO CONSIDER ITERATIONS FROM ONLY ITER 2,
        // THEN
        // X IS NOT A "REPEATED" CANDIDATE IN ITER 3. THEREFORE WE WANT TO KEEP
        // THE
        ...
        // Q: Why do we care? If we see the same candidate again, aren't we
        // going
        // to ignore it? So, why do we care about the SS of this repeat
        // candidate?
        ...
        for (int i = 0; i < numSentences; ++i) {
          ...
          for (int j = 1; j <= initsPerIt; ++j) {
            best1Score[j][i] = NegInf; // 1-BEST SCORE FOR EACH SENTENCE AND FOR
                                       // EACH CANDIDATE WEIGHT VECTOR
          }

          // PROCESS CANDIDATES FOR PREVIOUS ITERATIONS
          // LOW EFFICIENCY? FOR EACH ITERATION, IT READS IN ALL PREVIOUS
          // ITERATION OUTPUTS
          // THEREFORE A LOT OF OVERLAPPING JOBS
          // THIS IS AN EASY IMPLEMENTATION TO DEAL WITH THE SITUATION IN WHICH
          // USER ONLY SPECIFIED "prevIt" AND HOPES TO CONSIDER ONLY THE
          // PREVIOUS prevIt
          // ITERATIONS, THEN FOR EACH ITERATION THE EXISTING CANDADITES WILL BE
          // DIFFERENT
          for (int it = firstIt; it < iteration; ++it) {
            ...
            for (int n = 0; n <= sizeOfNBest; ++n) {
              // NOTE THAT IN ALL temp FILES, "||||||" IS A SEPARATOR BETWEEN 2
              // N-BEST LISTS
              ...
              // READ IN ONE LINE OF temp.sent, temp.feat, temp.stats FROM
              // ITERATION it
              ...
              } else if (!existingCandStats.containsKey(sents_str)) // IF THIS
                                                                    // CANDIDATE
                                                                    // DOES NOT
                                                                    // EXIST
              {
                  ...
                  } else // FOR MODE 4, NEED TO COMPUTE THE SUMMARY FEATURE VAL
                  {
                    ...
                    for (int c = 0; c < featVal_str.length; c++) {
                                                                          ...
                                                                          // FEATURE
                      {
                        ...
                        updated_feat_str += (featId + ":" + feat_info[1] + " "); // FEATURE
                                                                                 // ID
                                                                                 // ORDER
                                                                                 // DOESN'T
                                                                                 // MATTER
                    ...
                    feat_hash[i].put(sents_str, updated_feat_str); // UPDATE THE
                                                                   // FEATURE
                                                                   // HASH, NOW
                                                                   // THE SPARSE
                                                                   // FEATURES
                ...
                for (int j = 1; j <= initsPerIt; ++j) {
                  ...
                  if (score > best1Score[j][i]) {
                    best1Score[j][i] = score; // COMOPUTE 1-BEST MODEL SCORE
                                              // USING j^th WEIGHT VEC CANDIDATE
                    String[] tempStats = stats_str.split("\\s+");
                    for (int s = 0; s < suffStatsCount; ++s)
                      best1Cand_suffStats[j][i][s] = Integer.parseInt(tempStats[s]); // SUFF
                                                                                     // STAT
                                                                                     // s
                                                                                     // OF
                                                                                     // SENT
          ...
          // ---------- END OF MIRACESSING PREVIOUS ITERATIONS ----------
          // ---------- NOW START MIRACESSING NEW CANDIDATES ----------
          ...
          // which candidates (of the i'th source sentence) have not been seen
          // before
          // this iteration?
          ...
          for (int n = 0; n <= sizeOfNBest; ++n) {
            ...
            sents_str = inFile_sentsCurrIt.readLine(); // READ ONE CANDIDATE
                                                       // FROM THE CURRENT
                                                       // ITERATION
            ...
            if (sents_str.equals("||||||")) {
              ...
              unknownCands_V.add(sents_str); // NEW CANDIDATE FROM THIS
                                             // ITERATION
          ...
          // now unknownCands_V has the candidates for which we need to
          // calculate
        ...
        for (int it = firstIt; it < iteration; ++it) // PREVIOUS ITERATIONS TEMP
                                                     // FILES
        {
      ...
      if (newCandidatesAdded[iteration] == 0) {
        if (!oneModificationPerIteration) {
          ...
          println("", 1);
          return null; // THIS MEANS THAT THE OLD VALUES SHOULD BE KEPT BY THE
                       // CALLER
        } else {
      ...
      // FOR MODE 2&3: BOTH INITIAL/FINAL LAMBDA INCLUDE REGULAR+SPARSE FEATURE
      // WEIGHTS(REGULAR FEATURE WEIGHTS NOT UPDATED IN MODE 3)
      // FOR MODE 1&4: ONLY REGULAR FEATURE WEIGHTS ARE INCLUDED AND UPDATED
      // THIS NEEDS TO BE HANDLED BY Optimizer, AND FOR MODE 3, 1-BEST IS
      // COMPUTED ONLY ACCORDING TO DISC FEATURE WEIGHTS
      ...
      Optimizer.trainMode = trainingMode;
      Optimizer.nbestFormat = nbestFormat;
      ...
      Optimizer opt = new Optimizer(output, initialLambda[1], feat_hash, stats_hash, 
                                    score, numSparseParams, numParamsInFile);

      finalLambda[1] = opt.runOptimizer(); // FOR MODE 3, THE REGULAR WEIGHTS
                                            // ARE NOT UPDATED
      ...
      println("Scaling factor after this iteration: "+Optimizer.getScale());
      ...
      // if min number of iterations executed, investigate if early exit should
      // happen
      ...
      // USE THE NEW WT VECTOR TO DECODE THE NEXT ITERATION(INTERPOLATION WITH
      // PREVIOUS WT VECTOR)
  ...
  private void produceTempFiles(String nbestFileName, int iteration) {
    try {
      ...
      while (line != null) {
        ...
        // in a well formed file, we'd find the nth candidate for the ith
        // sentence
        ...
        line = (line.substring(line.indexOf("|||") + 3)).trim(); // get rid of
                                                                 // initial text
  ...
  private void createConfigFile(double[] params, String cfgFileName, String templateFileName) {
    try {
      ...
      if ((trainingMode.equals("2") || trainingMode.equals("3"))) {
        inFeatDefFile = new BufferedReader(new FileReader(discDefFile + ".MIRA.orig"));
      ...
      if (trainingMode.equals("1") || trainingMode.equals("4")) {
        ...
        // NOTE THAT THE DISCRIMINATIVE SUMMARY FEATURE WEIGHT IS SET TO 1.0, NO
        // MATTER WHAT NUMBER THE USER SPECIFIED
        ...
        while (line != null) {
          ...
          if (c_match == -1) {
            ...
            if (c_match == numParamsInFile)
              outFile.println(paramNames[c_match] + " " + 1.0); // DISC SUMMARY
                                                                // FEATURE
                                                                // WEIGHT
            else
              outFile.println(paramNames[c_match] + " " + params[c_match]);
  ...
  private void processParamFile() {
    ...
    for (int c = 1; c <= numParamsInFile; ++c) {
      ...
      } else // HANDLING DISC FEATURES IF IN MODE 2,3
      {
        if (c < numParamsInFile) {
          ...
          try {
            ...
            System.err.println("IOException in MIRACore.processParamFile(): " + e.getMessage());
            System.exit(99902);
          }

          for (int s = 0; s < numSparseParams; s++) {
            lambda[s + numParamsInFile] = sparseFeatWeights[s]; // IN MODE 2,3,
                                                                // ASSUMING DISC
                                                                // FEAT OVERALL
                                                                // WEIGHT = 1.0
      ...
      if (!isOptimizable[c]) { // skip next four values
        ...
        // set minThValue[c] and maxThValue[c] (range for thresholds to
        // investigate)
        dummy = inFile_init.next();
        if (dummy.equals("-Inf")) {
          minThValue[c] = NegInf;
    ...
    if (dummyA[0].equals("none")) {
      ...
      for (int i = 3; i < dummyA.length; ++i) { // in case parameter name has
                                                // multiple words
  ...
  private void processDocInfo() {
    ...
    if (docInfoFileName == null) {
      ...
      try {
        ...
        // 4 possible formats:
        // 1) List of numbers, one per document, indicating # sentences in each
        // document.
        // 2) List of "docName size" pairs, one per document, indicating name of
        // document and # sentences.
        // 3) List of docName's, one per sentence, indicating which doument each
        // sentence belongs to.
        // 4) List of docName_number's, one per sentence, indicating which
        // doument each sentence belongs to,
  ...
  public void finish() {
    ...
    if (trainingMode.equals("3") || trainingMode.equals("2")) {
      copyFile(discDefFile, discDefFile + ".MIRA.final");
      deleteFile(discDefFile);
      renameFile(discDefFile + ".MIRA.orig", discDefFile);
    }
          ...
          if (paramA.length == 2 && paramA[0].charAt(0) == '-') {
            ...
            argsVector.add(paramA[1]);
              ...
              && (paramA[0].equals("-m") || paramA[0].equals("-docSet") || paramA[0]
                  .equals("-damianos"))) {
            // -m (metricName), -docSet, and -damianos are allowed to have extra
            // optinos
  ...
  private void processArgsArray(String[] args, boolean firstTime) {
    ...
    while (i < args.length) {
      ...
      if (option.equals("-dir")) {
        ...
        maxMIRAIterations = Integer.parseInt(args[i + 1]);
        if (maxMIRAIterations < 1) {
          println("maxMIRAIts must be positive.");
          System.exit(10);
        }
      } else if (option.equals("-minIt")) {
        minMIRAIterations = Integer.parseInt(args[i + 1]);
        if (minMIRAIterations < 1) {
          println("minMIRAIts must be positive.");
          System.exit(10);
        }
      } else if (option.equals("-prevIt")) {
        prevMERTIterations = Integer.parseInt(args[i + 1]);
        if (prevMERTIterations < 0) {
          println("prevMIRAIts must be non-negative.");
          System.exit(10);
        }
      ...
      //SHULFFLE THE TRAINING SENTENCES OR NOT 
      ...
      //AVERAGE THE WEIGHTS AFTER EACH EPOCH OR NOT
    ...
    if (maxMIRAIterations < minMIRAIterations) {

      if (firstTime)
        println("Warning: maxMERTIts is smaller than minMERTIts; " + "decreasing minMERTIts from "
            + minMIRAIterations + " to maxMERTIts " + "(i.e. " + maxMIRAIterations + ").", 1);

      minMIRAIterations = maxMIRAIterations;
    }
    ...
    // TODO: also use this for the state file? could be tricky, since that file
    // is created by ZMERT.java
    ...
    if (!canRunCommand && !canRunJoshua) { // can only run fake decoder
      ...
      int lastGoodIt = 0;
      for (int it = 1; it <= maxMIRAIterations; ++it) {
      ...
      if (lastGoodIt == 0) {
        ...
        System.exit(13);
      } else if (lastGoodIt < maxMIRAIterations) {
        if (firstTime)
  ...
  private double[] randomLambda() {
    ...
    for (int c = 1; c <= numParams; ++c) {
      if (isOptimizable[c]) {
        ...
        ++generatedRands;
        randVal = randVal * (maxRandValue[c] - minRandValue[c]); // number in
                                                                 // [0.0,max-min]
  ...
  public static void main(String[] args) {
    ...
    MIRACore DMC = new MIRACore(); // dummy MIRACore object
    ...
    if (currIteration == 1) {
      ...
      DMC.println("----------------------------------------------------", 1);
      DMC.println("MIRA run started @ " + (new Date()), 1);
      ...
      try {
        ...
        System.err.println("FileNotFoundException in MIRACore.main(String[]): " + e.getMessage());
        System.exit(99901);
      } catch (IOException e) {
        System.err.println("IOException in MIRACore.main(String[]): " + e.getMessage());
        System.exit(99902);
      } catch (ClassNotFoundException e) {
        System.err.println("ClassNotFoundException in MIRACore.main(String[]): " + e.getMessage());
        System.exit(99904);
      }
      ...
      DMC.initialize(randsToSkip); // declares lambda[], sets numParams and
                                   // numSentences
    ...
    double[] A =
        DMC.run_single_iteration(currIteration, DMC.minMIRAIterations, DMC.maxMIRAIterations,
    ...
    if (A != null && A[2] != 1) {
      ...
      DMC.println("----------------------------------------------------", 1);
      DMC.println("MIRA run ended @ " + (new Date()), 1);
      // DMC.printMemoryUsage();
      DMC.println("----------------------------------------------------", 1);
      DMC.println("", 1);
      ...
      // delete intermediate .temp.*.it* decoder output files
      for (int iteration = 1; iteration <= DMC.maxMIRAIterations; ++iteration) {
