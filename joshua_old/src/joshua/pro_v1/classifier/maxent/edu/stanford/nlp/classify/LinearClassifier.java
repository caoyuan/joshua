...
import joshua.pro.classifier.maxent.edu.stanford.nlp.util.*;
...
public class LinearClassifier<L, F> implements ProbabilisticClassifier<L, F>, RVFClassifier<L, F> {
  ...
  protected List<Triple<F,L,Double>> getTopFeaturesLabelIndices(Set<Integer> iLabels,
                                                 ...
                                                 boolean descending)
  {
    joshua.pro.classifier.maxent.edu.stanford.nlp.util.PriorityQueue<Pair<Integer,Integer>> biggestKeys =
  ...
  public String toAllWeightsString()  // PREPARE FOR THE FINAL OUTPUT STRING
  {
    StringWriter sw = new StringWriter();
    PrintWriter pw = new PrintWriter(sw);
      ...
      Function<F, T> printer, boolean sortedByFeature) 
  {
    ...
    {  //FOR EACH FEATURE
      ...
      StringBuilder line = new StringBuilder(fStr);
      StringBuilder my_line = new StringBuilder(fStr);  //ONLY RECORD CLASS "1"
