...
import joshua.pro.classifier.maxent.edu.stanford.nlp.io.RuntimeIOException;
import joshua.pro.classifier.maxent.edu.stanford.nlp.ling.BasicDatum;
...
import joshua.pro.classifier.maxent.edu.stanford.nlp.process.WordShapeClassifier;
...
import joshua.pro.classifier.maxent.edu.stanford.nlp.stats.Counters;
import joshua.pro.classifier.maxent.edu.stanford.nlp.stats.Distribution;
import joshua.pro.classifier.maxent.edu.stanford.nlp.stats.TwoDimensionalCounter;
...
import joshua.pro.classifier.maxent.edu.stanford.nlp.util.Pair;
import joshua.pro.classifier.maxent.edu.stanford.nlp.util.ReflectionLoading;
import joshua.pro.classifier.maxent.edu.stanford.nlp.util.StringUtils;
import joshua.pro.classifier.maxent.edu.stanford.nlp.util.Triple;
 ...
 * Typical usage:
 * <p><code>
 * java joshua.pro.classifier.maxent.edu.stanford.nlp.classify.ColumnDataClassifier -prop propFile
 * </code><p>or<p>
 * <code>java -mx300m joshua.pro.classifier.maxent.edu.stanford.nlp.classify.ColumnDataClassifier
 ...
 * <tr><td>prior</td><td>String/int</td><td>quadratic</td><td>Type of prior (regularization penalty on weights). Possible values are null, "no", "quadratic", "huber", "quartic", "cosh", or "adapt". See {@link joshua.pro.classifier.maxent.edu.stanford.nlp.classify.LogPrior LogPrior} for more information.</td></tr>
...
public class ColumnDataClassifier {
  ...
  private static final Pattern tab = Pattern.compile("\\s");
  ...
  public GeneralDataset<String,String> readTrainingExamples(Vector<String> train_data) {
    ...
    //MODIFIED TO READ IN STRING VECTOR OF DATA DIRECTLY 
    //Pair<GeneralDataset<String,String>, List<String[]>> dataInfo = readDataset(fileName, false);
    Pair<GeneralDataset<String,String>, List<String[]>> dataInfo = readDataset(train_data, false);
   ...
   *  In test phase, returns the {@code List<String[]>} with the simple line infos for printing purposes.
    ...
    private void makeDatum(String cWord, Flags flags, Object featuresC, String goldAns) {
      ...
      if (flags.splitWordsRegexp != null || flags.splitWordsTokenizerRegexp != null ) {
        ...
        for (int i = 0; i < bits.length; i++) {
          ...
          if (flags.splitWordShape > joshua.pro.classifier.maxent.edu.stanford.nlp.process.WordShapeClassifier.NOWORDSHAPE) {
            String shape = joshua.pro.classifier.maxent.edu.stanford.nlp.process.WordShapeClassifier.wordShape(bits[i], flags.splitWordShape);
      ...
      if (flags.wordShape > WordShapeClassifier.NOWORDSHAPE) {
        String shape = joshua.pro.classifier.maxent.edu.stanford.nlp.process.WordShapeClassifier.wordShape(cWord, flags.wordShape);
   ...
   */
  public Classifier<String,String> makeClassifierAdaptL1(GeneralDataset<String,String> train){
    ...
    while (true) {
      ...
      Minimizer<DiffFunction> minim = ReflectionLoading.loadByReflection("joshua.pro.classifier.maxent.edu.stanford.nlp.optimization.OWLQNMinimizer", l1reg);
  ...
  public Classifier<String,String> makeClassifier(GeneralDataset<String,String> train) {
    ...
    if (globalFlags.useClassifierFactory != null) {
      ...
      LinearClassifierFactory<String,String> lcf;
      if (globalFlags.l1reg > 0.0) {
        Minimizer<DiffFunction> minim = ReflectionLoading.loadByReflection("joshua.pro.classifier.maxent.edu.stanford.nlp.optimization.OWLQNMinimizer", globalFlags.l1reg);
        lcf = new LinearClassifierFactory<String,String>(minim, globalFlags.tolerance, globalFlags.useSum, globalFlags.prior, globalFlags.sigma, globalFlags.epsilon);
      } else {
...
//    }
  ...
  private Flags[] setProperties(Properties props) {
    ...
    for (Enumeration<?> e = props.propertyNames(); e.hasMoreElements();) {
      ...
      if (key.equals("useString")) {
        ...
        System.err.println("Gold answer column is " + (myFlags[col].goldAnswerColumn));
   ...
   * java joshua.pro.classifier.maxent.edu.stanford.nlp.classify.ColumnDataClassifier -trainFile trainFile
  ...
  /*public static void main(String[] args) throws IOException {
    ...
    public static double[] run(String args, Vector<String> _train_data ) throws IOException 
    {
        ...
        internal_args[0] = "-prop";
        internal_args[1] = args;  //only specifies the prop file path
        internal_args[2] = "-trainFile";  //just a placeholder now
        internal_args[3] = "null";  //just a placeholder now
        internal_args[4] = "-testFile";  //just a placeholder now
        internal_args[5] = "null";  //just a placeholder now
  ...
  private double[] trainClassifier(Vector<String> train_data) throws IOException {
    ...
    if ((testFile == null && serializeTo == null) || trainFile == null) {
      System.err.println("usage: java joshua.pro.classifier.maxent.edu.stanford.nlp.classify.ColumnDataClassifier -prop propFile");
    ...
    if (Flags.printTo != null) 
    {
      ...
      try 
      {
        fw = new BufferedWriter(new FileWriter(Flags.printTo));
    ...
    else 
    {
      ...
      System.err.println("---------"+classString+"*********");  //THIS IS THE FINAL OUTPUT ON SCREEN
    }*/
  ...
  static class Flags implements Serializable {
    ...
    int wordShape = joshua.pro.classifier.maxent.edu.stanford.nlp.process.WordShapeClassifier.NOWORDSHAPE;
