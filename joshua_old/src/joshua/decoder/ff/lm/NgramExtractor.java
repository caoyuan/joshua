	 ...
	 private HashMap<String,Integer> computeTransitionNgrams(Rule rule, List<HGNode> antNodes, int startNgramOrder, int endNgramOrder){
			...
			for(int c=0; c<enWords.length; c++)
			{
	    		...
	    		if(symbolTable.isNonterminal(curID)==true){//non-terminal words
	    			...
	    			HGNode antNode =  antNodes.get(index);  //GET THE LEFT/RIGHT ANTECEDENT NODE OF THIS HYPEREDGE 
			...
			for(Map.Entry<String, Integer> entry : newNgramCounts.entrySet())
			{
	    		...
	    		int finalCount = entry.getValue();  //THE finalCount = OLD COUNT + NEW COUNT(IF THE NGRAM IS NEW)
	    		...
	    		if(finalCount>0)
	    			res.put(ngram, finalCount);  //SAVE THE NGRAM COUNT
	    	}
	    return res;
	}	
