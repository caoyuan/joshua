 ...
 * @version $LastChangedDate: 2011-03-16 00:59:38 -0400 (Wed, 16 Mar 2011) $
 */
public class KBestExtractor {
	...
	// Note that extractNbestTree only affects the output format, not the uniquness of kbest.
...
//	}
	...
	public String getKthHyp(HGNode it, int k,  int sentID, List<FeatureFunction> models, int[] numNodesAndEdges) {
		...
		//System.out.println("SentID: " + sentID + " K-best: " + k);
		...
		if( cur==null) 
			...
			//MODIFY modelCost TO INCLUDE ALL DISC FEATURES! 
			...
			//System.out.println("getKthhyp ------");
			
			clearFeatCount();
			//firing_feat_counts = new HashMap<Integer, Integer>();
			
			String strHypNumeric = cur.getHypothesis(symbolTable, this, extractNbestTree, modelCost, models, numNodesAndEdges);
			
			//System.out.println("getKthHyp: " + strHypNumeric );
			
			//for(int s=0; s<modelCost.length; s++) System.out.println(modelCost[s]);
			
			//OUTPUT THE N-BEST-FORMAT STRING: X ||| X ||| X 
			
			/*for( int i=0; i<models.size(); i++ )
			{
				System.out.println("model: "+models.get(i).getFeatureID()+" "+models.get(i).getWeight());
				System.out.println("modelCost: "+modelCost[i]);
			}*/
				
			String strHypStr = convertHyp2String(sentID, cur, models, strHypNumeric, modelCost);
			
			//System.out.println(strHypStr);
			return strHypStr;
		}
	}
	...
	public HGNode  getKthHyp(HGNode it, int k, int[] numNodesAndEdges) {
		...
		//System.out.println("KBestExtractor^^^^^^^^^^^^^^ " + k + " " + sentID);
		
		//==== setup the kbest at each hgnode
		VirtualNode virtualNode = addVirtualNode(it);
		DerivationState cur = virtualNode.lazyKBestExtractOnNode(symbolTable, this, k);
		
		//System.out.println("KBestExtractor*************** ");
	...
	*/
		...
		CoIterator<String> coit) 
	{
		...
		try {
			...
			while (true) {
				...
				//FEATURE FUNCTIONS SAVES ALL DISC FEATURES
				String hypStr = getKthHyp(hg.goalNode, ++nextN, sentID, featureFunctions, null);
				
				//System.out.println("lazyKBestExtractOnHG: "+sentID+" " +hypStr);
	...
	//modelCost SAVES ALL FEATURE VALUES
	private String convertHyp2String(int sentID, DerivationState cur, List<FeatureFunction> models, String strHypNumeric, double[] modelCost)
	{
		...
		if (null != modelCost) {
			...
			/*for (int k = 0; k < modelCost.length; k++) 
			{
				...
				//System.err.println("tem_sum: " + temSum + " += " + modelCost[k] + " * " + models.get(k).getWeight());
			}*/
			...
			/*******************************************************
				WARING!!
				THIS VERSION IS MEANT TO OUTPUT FIRING SPARSE FEATURES, THAT IS IF IN THE joshua.config FILE, THE SPARSE
				FEATURES ARE SPECIFIED LIKE:
				discriminative micro.rule.feat 1.0
				THEN THIS WEIGHT(1.0) WILL BE IGNORED! ONLY SPARSE FEATURE WEIGHTS WILL BE OUTPUT
				IF YOU WANT TO OUTPUT THIS FEATURE, REPLACE
				modelCost.length-1
				TO
				modelCost.length
			 ******************************************************/ 
			...
			while(it.hasNext())
			{
				...
				strHyp.append(String.format(" %d:%d", feat_id+modelCost.length, firing_feat_counts.get(feat_id)));
			}
...
//===============================================
	...
	private class DerivationState implements Comparable<DerivationState> 
	{
				...
				List<FeatureFunction> models,  int[] numNodesAndEdges) 
		{
			...
			//for( int i=0; i<modelCost.length; i++ )
			//	System.out.print(modelCost[i]+" ");
		...
		*/
		...
		private void computeCost(HGNode parentNode, HyperEdge dt, double[] modelCost, List<FeatureFunction> models){
			...
			//System.out.println("Rule is: " + dt.getRule());
			...
			//System.out.println("computeCost+++: " + modelCost[10] + " " + transitionCosts[10]);
			//System.out.println();
		}
	...
	public static void increaseFeatCount( Integer feat_id )  //CALLED BY FeatureTemplateBasedFF:getTransitionLogP()
	{
